...
ssh-agent ssh-add '/run/media/' + ([System.Environment]::UserName) + '/4b2478fd-5b2b-4927-91fb-19d9c9ef3087/entit/beedell.rokejulianlockhart/1662394226/keys/$(ssh-keygen)/.(publik.negativ).bin.txt'; git clone --bare "git@$(Get-Variable -Name 'ChosenDomain' | Select-Object -ExpandProperty Value):$(Get-Variable -Name 'Identifier' | Select-Object -ExpandProperty Value)/$(Get-Variable -Name 'Identifier' | Select-Object -ExpandProperty Value).git")
Set-Location "$(Get-Variable -Name 'Identifier' | Select-Object -ExpandProperty Value).git"}
ssh-agent $(ssh-add '/run/media/' + ([System.Environment]::UserName) + '/4b2478fd-5b2b-4927-91fb-19d9c9ef3087/entit/beedell.rokejulianlockhart/1662394226/keys/$(ssh-keygen)/.(publik.negativ).bin.txt'; git push --mirror "git@$(Get-Variable -Name 'ChosenDomain' | Select-Object -ExpandProperty Value):$(Get-Variable -Name 'Identifier' | Select-Object -ExpandProperty Value)/$(Get-Variable -Name 'Identifier' | Select-Object -ExpandProperty Value).github.io.git")

# Origin: https://stackoverflow.com/a/4565746/9731176
